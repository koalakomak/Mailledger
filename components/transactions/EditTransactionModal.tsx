...
export function EditTransactionModal({ transaction, onClose, onSuccess }: Props) {
  ...
  // Tutup dengan tombol Escape (aksesibilitas keyboard)
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" role="presentation">
      <div
        ...
        className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in-95"
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <h3 id="edit-tx-title" className="font-semibold text-white">Edit Detail Transaksi</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
          {error && (
            <div role="alert" className="flex items-center gap-2 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs">
