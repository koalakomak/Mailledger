  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm max-w-2xl">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-semibold text-base text-white">Integrasi Google Spreadsheet</h3>
          <p className="text-xs text-slate-400">
            Otomatis ekspor dan tambahkan transaksi hasil parsing ke Google Sheet Anda.
          </p>
        </div>
      </div>
      ...
      {connection?.isActive && (
        <div className="mb-6 p-4 rounded-lg bg-emerald-950/40 border border-emerald-500/30 text-xs text-emerald-300">
          <div className="flex items-center justify-between">
            <span className="font-semibold">✓ Saat ini terhubung ke Google Sheet</span>
            <a
            ...
            >
              Buka Sheet <ExternalLink className="w-3.5 h-3.5" />
            </a>
          </div>
          <div className="mt-2 text-slate-400">
            Nama sheet: <span className="text-slate-200 font-mono">{connection.sheetName}</span> | Sinkron terakhir:{" "}
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-sm">
        ...
        <div className="pt-2">
          ...
          >
            ...
            {isSubmitting ? "Menghubungkan..." : connection?.isActive ? "Perbarui Koneksi Spreadsheet" : "Hubungkan Spreadsheet"}
          </button>
        </div>
      </form>
    </div>
